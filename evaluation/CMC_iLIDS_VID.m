...
datasetPath = '~/cloud/zangxh/iLIDS-VID/i-LIDS-VID/sequences/';
imgsize = [128, 64, 3];
normalization = 1;
% CMC of 10 splits
all_CMC = [];
for split_count = 1:10
    ...
    feature_path_1 = strcat('../results/iLIDS-VID/', model_name_, '/local/eight_part/features/', split, '/');
    feature_path_2 = strcat('../results/iLIDS-VID/', model_name_, '/global/eight_part/features/', split, '/');
    ...
    path = strcat('datasplits/iLIDS-VID/', split, '/testID.mat');
    testID = importdata(path); % personID
    path = strcat('datasplits/iLIDS-VID/', split, '/testCAM.mat');
    testCAM = importdata(path); % camID
    path = strcat('datasplits/iLIDS-VID/', split, '/testdata.txt');
    [testimgs,b,c,d] = textread(path, '%s%n%n%n'); % imgPath, personID, camID, seqID
    testpersonID = cellfun(@(x) x(12:14), testimgs, 'UniformOutput', false); % extract personID from imgPath
    testpersonID = unique(testpersonID, 'stable');

    % For iLIDS-VID, take either the first or the second cam as the probe set 
    % then compute averaged results
    mean_CMC = [];
    for flip = 0:1
        if flip ==1
        ...
        CMC = mean(CMC);
        %% store results
        mean_CMC = [mean_CMC; CMC];
    end
    % averaged results of two type of probe & gallery combination
    CMC = mean(mean_CMC);
    all_CMC = [all_CMC; CMC];
